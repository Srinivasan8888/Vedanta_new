...
export const createSetAlert = async (req, res) => {
    ...
    const phoneNoString = String(phoneNo).replace(/\D/g, '').slice(0, 10);
    ...
    try {
        ...
        const existingEmployee = await setAlert.findOne({ employeeNo });
        if (existingEmployee) {
            return res.status(400).json({ 
                message: 'This employee number is already registered. Please use a different employee number.' 
            });
        }

        console.log('Attempting to create alert user with data:', { 
            name, 
            email, 
            employeeNo, 
        ...
        const datas = await setAlert.create({ 
            name, 
            email, 
            employeeNo, 
            phoneNo: phoneNoString 
