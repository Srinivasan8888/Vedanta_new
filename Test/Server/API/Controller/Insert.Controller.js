...
export const createSensor1 = async (req, res) => {
  ...
  export const createSensor4 = async (req, res) => {
    const { id, busbar, sensor29, sensor30, sensor31, sensor32, time } = req.query;
  
    if (!id || !busbar || !sensor29 || !sensor30 || !sensor31 || !sensor32 || !time) {
      return res.status(400).json({
        error: "All fields are required.",
      });
    }
  ...
  //sensor6
  export const createSensor6 = async (req, res) => {    const {
      id,
      busbar,
      sensor39,
      sensor40,
